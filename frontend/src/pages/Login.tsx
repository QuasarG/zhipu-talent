  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-8 bg-surface">
      <Card variant="elevated" className="relative w-full max-w-[400px] p-8 text-center">
        {/* 语言切换：钉在卡片右上角（外层 span 负责定位，避免与按钮自身布局类冲突） */}
        <span className="absolute top-4 right-4">
          <LangToggle className="h-8 px-3 rounded-md border border-outline-variant" />
        </span>
        <img
          src={lang === "en" ? (resolved === "dark" ? logoEnWhiteUrl : logoEnUrl) : resolved === "dark" ? logoWhiteUrl : logoUrl}
          alt={t("智谱")}
          className="h-7 mx-auto mb-4 select-none"
          draggable={false}
        />
        <h1 className="text-headline mt-0 mb-2">{t("智谱人才研究平台")}</h1>
