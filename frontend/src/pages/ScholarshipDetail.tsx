...
export default function ScholarshipDetail() {
  ...
  const runScan = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "舆情扫描失败");
    } finally {
      setScanning(false);
    }
  };
  ...
  if (notFound) {
    return (
      <div className="flex flex-col items-center gap-3 py-20">
        <p className="text-body-sm text-error">{error || "申请人不存在"}</p>
        <Button variant="tonal" icon="arrow_back" onClick={() => navigate("/scholarship")}>返回列表</Button>
      </div>
    );
  }

  if (!app) {
    return <div className="flex justify-center py-20"><LoadingIndicator size={28} label="加载中…" /></div>;
  }
  ...
  return (
    <div className="w-full max-w-full min-h-0 flex flex-col gap-4 pb-6">
      ...
      <div className="flex items-start gap-3 px-2 pt-3">
        <IconButton icon="arrow_back" variant="outlined" onClick={() => navigate("/scholarship")} title="返回列表" />
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-headline leading-tight">{app.name || "未命名"}</h1>
            <ScholarshipStatusChip status={app.status} />
          </div>
          <p className="text-body-sm text-on-surface-variant mt-1">
            {[app.school, DEGREE_LABELS[app.degree_type] ?? app.degree_type, app.direction, app.expected_graduation]
              .filter(Boolean).join(" · ") || "—"}
            {app.advisors.length > 0 && ` · 推荐导师：${app.advisors.join("、")}`}
          </p>
        </div>
      </div>
      ...
      <Card variant="filled" className="p-4 flex flex-col gap-3">
        <p className="text-title-lg">品牌加分</p>
        ...
        {app.brand_note && (
          <p className="text-label text-on-surface-variant">当前：{fmtAdjust(app.brand_bonus)} 分 · {app.brand_note}</p>
        )}
      </Card>
      ...
      <Card variant="filled" className="p-4 flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <p className="text-title-lg">评分明细</p>
          {latestCompleted && (
        ...
        {latestEval?.status === "running" ? (
          <LoadingIndicator size={20} label="评估中…" />
        ) : latestEval?.status === "failed" ? (
          <p className="text-body-sm text-error">评估失败：{latestEval.error_message || "未知错误"}</p>
        ) : !latestCompleted ? (
          <p className="text-body-sm text-on-surface-variant">暂无评估结果，回列表点「评估」</p>
        ) : (
          <>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-label text-on-surface-variant mb-1.5">亮点</p>
              ...
              <div>
                <p className="text-label text-on-surface-variant mb-1.5">风险</p>
                <ul className="flex flex-col gap-1">
          ...
          <div className="flex flex-col gap-2">
            {(app.reputation_items ?? []).map((item) => (
              <div key={item.id} className="border border-outline-variant rounded-lg p-3 flex flex-col gap-1.5">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-body-sm font-medium text-on-surface">
                    ...
                    <span className="ml-1 text-label text-on-surface-variant">
                      {SUBJECT_ROLE_LABELS[item.subject_role] ?? item.subject_role}
                    </span>
                  </span>
                  <StatusChip
                  ...
                  >
                    {item.sentiment === "negative" ? "负面" : "正面"}
                  </StatusChip>
                  {item.review_status !== "pending" && (
                    <StatusChip tone={item.review_status === "confirmed" ? "warning" : "neutral"}>
                      {REVIEW_LABELS[item.review_status] ?? item.review_status} {fmtAdjust(item.adjustment)}
                    </StatusChip>
                  )}
                </div>
                <a
                ...
                {item.concern && <p className="text-body-sm text-error">风险点：{item.concern}</p>}
                {item.review_status === "pending" && (
      ...
      {/* 材料 */}
      <Card variant="filled" className="p-4 flex flex-col gap-3">
        <p className="text-title-lg">申请材料（{app.materials?.length ?? 0}）</p>
        {groupedMaterials.length === 0 ? (
          <p className="text-body-sm text-on-surface-variant">暂无材料</p>
        ) : (
          groupedMaterials.map(([kind, items]) => (
            <div key={kind} className="flex flex-col gap-1.5">
              <p className="text-label text-on-surface-variant">
                {MATERIAL_KIND_LABELS[kind] ?? kind}（{items.length}）
              </p>
              {items.map((m) => (
                <details key={m.id} className="border border-outline-variant rounded-lg px-3 py-2 group">
                  <summary className="text-body-sm text-on-surface cursor-pointer select-none flex items-center gap-2">
                    ...
                    {m.kind === "letter" && m.advisor_name && (
                      <span className="text-label text-on-surface-variant">推荐人：{m.advisor_name}</span>
                    )}
                  </summary>
