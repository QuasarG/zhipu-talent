    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40" onClick={onClose}>
      ...
      >
        <div className="flex items-center gap-2 px-5 py-3 border-b border-outline-variant shrink-0">
          ...
          <h2 className="text-title font-bold text-on-surface">分组管理</h2>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4 flex flex-col gap-2">
          {groups.length === 0 ? (
            <p className="text-center py-6 text-body-sm text-on-surface-variant">还没有分组，在下面新建一个吧</p>
          ) : (
            groups.map((g) => (
              <div key={g.id} className="flex items-center gap-2 px-3 py-2 rounded-md hover:bg-surface-low">
                ...
                <span className="text-label text-on-surface-variant">{g.count} 人</span>
                {confirmingId === g.id ? (
                  <>
                    <button disabled={busy} onClick={() => remove(g.id)} className="px-2 h-7 rounded-full text-label text-error hover:bg-error-container cursor-pointer">确认删除</button>
                    <button onClick={() => setConfirmingId(null)} className="px-2 h-7 rounded-full text-label text-on-surface-variant hover:bg-surface-low cursor-pointer">取消</button>
                  </>
                ) : (
                  <>
                    <button
