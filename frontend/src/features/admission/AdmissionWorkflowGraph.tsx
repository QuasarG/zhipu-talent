          ...
          >
            ...
            >
              {node.kind === "source" && !node.icon ? (
                <span className="initial-avatar font-medium" style={{ fontSize: Math.max(13, node.screenRadius * 0.68) }}>{(candidate?.display_name || candidate?.name || t("候选人")).slice(0, 1)}</span>
              ) : level !== null ? (
