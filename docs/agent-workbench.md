...
- 候选人评估（唯一链路，panel）：主席是带工具的主 agent，通过 `spawn_agent` 派出子评审 agent（spawn prompt 全权约束、不可嵌套），子 agent 工具与主 agent 相同（材料只读）。评估过程以单条 assistant 消息呈现（奖学金同款），子 agent 为内联 spawn 段。
