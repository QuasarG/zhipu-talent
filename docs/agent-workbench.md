...
- 候选人评估的 panel 模式：主席动态派发 verify、deep_read、jd_match、cross_check、generic 任务，当前实现逐个执行，可续派同一任务上下文。
- 面试准入批次：能力映射、并行任务评分、证据修复、独立总审、确定性系统裁决；不受 TALENT_EVALUATION_MODE=panel 切换。
