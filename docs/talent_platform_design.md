# 智谱人才情报 · 后端功能与 Agent 节点设计

> 状态：设计稿 v2（三功能产品结构，见 `TODO.md`）。
> 定位：智谱书院内部工具。三个边界独立的功能 + 一个共享 `persons` 主档底座。
> 前身：`agi_talent_radar` 简历初筛系统（OCR 解析、多 Track 评分、校准机制全部复用）。

## 1. 产品定位与架构决策

**一句话**：三个独立功能共享一个人员主档底座，分别为嘉宾核查、简历初筛、人才库查询提供服务。
...
**核心架构决策**：评分、匹配、舆情三者量纲不同，**永远分开**，不做揉合的「决策组装」节点。

| 量纲 | 归属 | 说明 |
|---|---|---|
| 能力评分（绝对值） | 功能② 简历初筛 | 只评「这个人本身有多强」 |
| 匹配程度（相对值） | 独立匹配模块 | 只评「跟岗位有多契合」，等 HRBP 需求，不阻塞② |
| 舆情风险（闸门） | 功能① 嘉宾画像 | 红黄绿分级，不进分数加减 |
...
| 功能 | 输入 | 输出 | 模式 |
|---|---|---|---|
| ① 嘉宾画像+舆情 | 姓名 + 机构（+可选方向） | 研究方向画像 + 代表成果核查(✓/✗) + 舆情分级(红黄绿+来源) | `guest_check` |
| ② 简历初筛 | 简历 PDF | 能力评分 + 画像 + 面谈追问（不接舆情、不做匹配、不做决策闸门） | `full_eval` |
| ③ 人才库 | 无（纯查询） | 评估过的嘉宾 + 简历列表/详情（含历史归并、舆情状态） | 查询 |

> 设计稿后续章节（3-10 节）保留原节点详设与数据模型作为实现参考，
> 但分期（P1-P5）已废弃，改为按 `TODO.md` 的功能①→③→②顺序推进。
